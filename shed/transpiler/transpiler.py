...
After retokenizer processes source code, we have a python code with
 top-level 'statements' (`"$(ls)"`) or autogenerated assigns (`__script_var_1 = "$(ls)"`)
 in place of shell calls.
Transpiler goes through AST and transforms those statements into `_impl` calls.
...
class ShellCallTransformer(NodeTransformer):
    ...
    def visit_Assign(self, node: Assign) -> Any:
        ...
        if not isinstance(node.targets[0], Name) or len(node.targets) != 1:
            return node
        if self.context.is_autogenerated(node.targets[0].id):  # type:ignore
